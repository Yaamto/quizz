...
const Result = ({usersAnswers, room, user, nextQuestion, socket}: Props) => {
    ...
    return (
         ...
         <div className='font-bold flex flex-col items-center justify-center gap-2'>
          ...
          <Image
                  src={`/${users?.reduce((prev, current) => (prev.point > current.point) ? prev : current).avatar}.jpg`}
                  alt="@shadcn"
                  width={45}
                  height={45}
         ...
         <p>Points: {users?.reduce((prev, current) => (prev.point > current.point) ? prev : current).point}</p>
         </div>
         </div>
         )}
        ...
        <p className='my-2 bg-blue-500 text-white py-2 px-4 rounded'>Question {room?.quizz?.length }: {room?.quizz?.[room?.quizz?.length - 1].question} </p>
        ...
        <ul>
          {usersAnswers?.map((userAnswer: any, idx: number) => (
            <li key={idx} className={`flex items-center gap-3 py-2 px-4 rounded border my-2 ${userAnswer?.answer === room?.quizz?.[room?.quizz?.length -1].answer ? "border-green-600" : "border-red-600"} `} >
              ...
              {users.length > 0 && users?.map((client: any) => {
        ...
        {room?.nbQuestions === room?.quizz?.length && <Link href="/room/join" className="mt-2"><Button>Back to rooms</Button></Link>}
        </div>
    );
};
